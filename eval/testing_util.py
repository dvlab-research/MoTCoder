...
def run_test(sample, test=None, debug=False):
    ...
    if test is None:
        ...
        if which_type == CODE_TYPE.call_based:
            ...
            try:
                ...
                # outputs.append(f"Type 0 compilation error = {e}")
                results.append(-2)
            ...
            try:
                ...
                # outputs.append(f"Type 1 Compilation error: {e}")
                results.append(-2)
                return results
            signal.alarm(0)
        if debug:
        ...
        try:
            ...
            results.append(-2)
            # outputs.append(f"Unable to get function error: {e}")
            return results
        ...
        for index, inputs in enumerate(in_outs["inputs"]):
            ...
            if which_type == CODE_TYPE.call_based:  # Call-based
                ...
                try:
                    ...
                    # outputs.append('Standard input runtime error or time limit exceeded error: {e}')
                    results.append(-1)
                    continue
                faulthandler.disable()
                signal.alarm(0)
                ...
                with Capturing() as output:
                    try:
                        ...
                        results.append(-1)
                        # outputs.append(f"Call-based runtime error or time limit exceeded error = {repr(e)}{e}")
                        reflection.append(f"Call-based runtime error or time limit exceeded error = {repr(e)}{e}")
                    signal.alarm(0)
                ...
                if not passed:
                    ...
                    if not isinstance(inputs, list):
                        ...
                        reflection.append(f"not passed output = {output}, test outputs = {in_outs['outputs'][index]}, inputs = {inputs.replace(nl,' new-line ')}, {type(inputs)}, {output == [in_outs['outputs'][index]]}")
                    else:
                        if debug:
                        ...
                        reflection.append(f"not passed output = {output}, test outputs = {in_outs['outputs'][index]}, inputs = {inputs}, {type(inputs)}, {output == [in_outs['outputs'][index]]}")
                    continue
                ...
                if not isinstance(inputs, list):
                    if debug:
                        print(f"output = {output}, test outputs = {in_outs['outputs'][index]}, inputs = {inputs.replace(nl,' new-line ')}, {type(inputs)}, {output == [in_outs['outputs'][index]]}")
                    reflection.append(f"output = {output}, test outputs = {in_outs['outputs'][index]}, inputs = {inputs.replace(nl,' new-line ')}, {type(inputs)}, {output == [in_outs['outputs'][index]]}")
                else:
                    if debug:
                        print(f"output = {output}, test outputs = {in_outs['outputs'][index]}, inputs = {inputs}, {type(inputs)}, {output == [in_outs['outputs'][index]]}") 
                    reflection.append(f"output = {output}, test outputs = {in_outs['outputs'][index]}, inputs = {inputs}, {type(inputs)}, {output == [in_outs['outputs'][index]]}") 
    ...
    return {"outputs": reflection, "results": results}
...
def reliability_guard(debug=False, maximum_memory_bytes=None):
    """
